...
public class DownloadPartnerSchools extends HttpServlet {
	...
	private static final Logger log = Logger.getLogger(DownloadPartnerSchools.class.getName());
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getRemoteHost().contains("localhost") || request.getRemoteHost().contains("127.0.0.1")) {
		...
		List<PartnerSchoolEntity> schoolList = patss.getPartnerByInstitute(insId, yearofExam);
		...
		try {
			...
			response.setHeader("Content-Disposition", "attachment; filename=SchoolData_" + sdf.format(date) + ".csv");
			...
			writer.append(',');
			writer.append("School Total Students(JFI)");
			writer.append(',');
			writer.append("Male");
			writer.append(',');
			...
			// Part-3
			String standardList[] = { "5th", "6th", "7th", "8th", "9th", "10th", "11th", "12th", "FY", "SY", "TY",
					"Fr. Y", "PG/D. & B. Ed-1", "PG/D. & B. Ed-2", "Teacher" };

			for (String std : standardList) {
				writer.append(std);
				writer.append(',');
			}
			writer.append("Paid Student Total");
			// Part-4
			...
			writer.append(',');
			writer.append("Free Student Waived Fees");
			writer.append(',');
			writer.append("Total Student(Paid + Free )");
			writer.append(',');
			...
			for (int i = 0; i < schoolList.size(); i++) {
				...
				try {
					...
					writer.append(',');
					writer.append(UtilityService.trimForCSV(schoolEntity.getSchoolName()));
					writer.append(',');
					...
					String formNumber = schoolEntity.getFormNumber();
					...
					writer.append(UtilityService.trimForCSV(schoolEntity.getAddress().getLine1()));
					writer.append(',');
					writer.append(UtilityService.trimForCSV(schoolEntity.getAddress().getLine2()));
					writer.append(',');
					String city = UtilityService.trimForCSV(schoolEntity.getAddress().getCity());
					writer.append(city);
					writer.append(',');
					...
					String headMasterName = UtilityService
							.trimForCSV(schoolEntity.getContactDetail().getHeadMasterName());
					writer.append(headMasterName);
					writer.append(',');
					writer.append(schoolEntity.getContactDetail().getHeadMasterMobile());
					writer.append(',');
					writer.append(schoolEntity.getContactDetail().getHeadMasterEmailId());
					writer.append(',');

					if (schoolEntity.getContactDetail().getCoordinatorDetail() != null) {
						String coordinatorName = schoolEntity.getContactDetail().getCoordinatorDetail().get(0)
								.getCoordinatorName();
						coordinatorName = UtilityService.trimForCSV(coordinatorName);
						writer.append(coordinatorName);
						writer.append(',');
						writer.append(schoolEntity.getContactDetail().getCoordinatorDetail().get(0)
								.getCoordinatorMobileNum());
						writer.append(',');
						writer.append(
								schoolEntity.getContactDetail().getCoordinatorDetail().get(0).getCoordinatorEmailId());
						writer.append(',');
					}
					...
					if (examDeatil != null) {
						// Part-2
						List<PaymentDetail> paymentDetailList = examDeatil.getPaymentDetail();
						if (paymentDetailList != null && !paymentDetailList.isEmpty()) {
							...
							String payReceivedBy = UtilityService
									.trimForCSV(paymentDetailList.get(0).getPayReceivedBy());
							writer.append(payReceivedBy);
							writer.append(',');

							String nameOfBank = UtilityService.trimForCSV(paymentDetailList.get(0).getNameOfBank());
							writer.append(nameOfBank);
							writer.append(',');

							String nameOfBranch = UtilityService.trimForCSV(paymentDetailList.get(0).getBranchName());
							writer.append(nameOfBranch);
							writer.append(',');

							writer.append(paymentDetailList.get(0).getTransactionNumber());
							writer.append(',');

							SimpleDateFormat sd = new SimpleDateFormat("dd/MM/yyyy");
							sd.setTimeZone(TimeZone.getTimeZone("IST"));
							String dateStr = sd.format(paymentDetailList.get(0).getPaymentDate());
						...
						writer.append(',');
						writer.append(examDeatil.getMale() == null ? "" : examDeatil.getMale());
						writer.append(',');
						writer.append(examDeatil.getFemale() == null ? "" : examDeatil.getFemale());
						writer.append(',');
						...
						for (String std : standardList) {
							...
							writer.append(Integer.toString(studentsByStandard));
							writer.append(',');
						}
						// Part-4
						writer.append(paidStudTotal + "");
						//Extra column for Free stud blank column
						writer.append(',');
						int freeStudTotal = 0;
						for (String std : standardList) {
							int freeStudentsByStandard = getFreeStudentsByStandard(std, bookDetailList);
							freeStudTotal += freeStudentsByStandard;
							writer.append(Integer.toString(freeStudentsByStandard));
							writer.append(',');
						}
						writer.append(freeStudTotal + "");
						writer.append(',');
						int freeStudTotalFees = 0;
						...
						writer.append(paidPlusFreeTotal + "");
						writer.append(',');
