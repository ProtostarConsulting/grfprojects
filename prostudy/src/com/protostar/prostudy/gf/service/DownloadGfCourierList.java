...
public class DownloadGfCourierList extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		List<PartnerSchoolEntity> schoolList = patss.getPartnerByInstitute(insId);
		List<GFCourierEntity> courierList = gfCourierService.getGFCourierByInstitute(insId);
		...
		try {
			...
			for (int i = 0; i < courierList.size(); i++) {
				...
				writer.append(',');
				writer.append(courierList.get(0).getAutoGenerated());
				writer.append(',');
				
				PartnerSchoolEntity schoolEntity = schoolList.get(i);
				
				writer.append(schoolEntity.getSchoolName().replace(',', '-'));
				writer.append(',');
				...
				String dateStr = sd.format(courierList.get(0).getCourierDispatchDate());
						
				writer.append(dateStr);
				writer.append(',');
				writer.append(courierList.get(0).getLogistics());
				writer.append(',');
				writer.append(courierList.get(0).getCourierName());
				writer.append(',');
				writer.append(courierList.get(0).getCourierDocketID());
				writer.append(',');
			/*	writer.append(Integer.toString(courierList.get(0).getCourierCost()));
				writer.append(',');*/
