...
public class PartnerSchoolService {
	...
	@ApiMethod(name = "touchAllSchools")
	public void touchAllSchools() throws MessagingException,
			IOException {
	...
	public List<PartnerSchoolEntity> searchSchoolByName(
			@Named("nameSearchString") String nameSearchString) {
		List<PartnerSchoolEntity> schoolList = ofy().load()
				.type(PartnerSchoolEntity.class).filter("schoolNameIndex in", nameSearchString.trim().split(" "))
				.order("-autoGenerated").list();
	...
	public EntityPagingInfo fetchSchoolsListByPaging(
		...
		Query<PartnerSchoolEntity> filterInstituteQuery = ofy().load()
				.type(PartnerSchoolEntity.class).limit(pagingInfo.getLimit())
				.filter("instituteID", id).order("-autoGenerated");

		if (pagingInfo.getWebSafeCursorString() != null)
			filterInstituteQuery.startAt(Cursor.fromWebSafeString(pagingInfo
					.getWebSafeCursorString()));
		...
		pagingInfo.setTotalEntities(ofy().load()
				.type(PartnerSchoolEntity.class).filter("instituteID", id).count());
