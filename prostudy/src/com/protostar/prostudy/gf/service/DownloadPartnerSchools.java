			...
			for (int i = 0; i < schoolList.size(); i++) {
				...
				try {
					...
					writer.append(schoolEntity.getAddress().getLine2()
							.replace(',', '-'));
					writer.append(',');
					String city = schoolEntity.getAddress().getCity();
					city = city.replace("\n", "").replace("\r", "").trim();
					writer.append(city != null ? city.replace(',', '-') : "");
					writer.append(',');
					...
					writer.append(',');
					String headMasterName = schoolEntity.getContactDetail()
							.getHeadMasterName();
					headMasterName = headMasterName.replace("\n", "").replace("\r", "").trim();
					writer.append(headMasterName != null ? headMasterName.replace(',', '-') : "");
					writer.append(',');
					...
					if (schoolEntity.getContactDetail().getCoordinatorDetail() != null) {
						String coordinatorName = schoolEntity.getContactDetail()
								.getCoordinatorDetail().get(0)
								.getCoordinatorName();
						coordinatorName = coordinatorName.replace("\n", "").replace("\r", "").trim();
						writer.append(coordinatorName != null ? coordinatorName.replace(',', '-') : "");
						writer.append(',');
						writer.append(schoolEntity.getContactDetail()
								.getCoordinatorDetail().get(0)
					...
					if (examDeatil != null) {
						...
						if (paymentDetailList != null
							...
							String payReceivedBy = paymentDetailList.get(0)
									.getPayReceivedBy().replace("\n", "")
									.replace("\r", "").trim();
							writer.append(payReceivedBy);
							writer.append(',');

							String nameOfBank = paymentDetailList.get(0)
									.getNameOfBank().replace("\n", "")
									.replace("\r", "").trim();
							writer.append(nameOfBank != null ? nameOfBank
									.replace(',', '-') : "");
							writer.append(',');

							String nameOfBranch = paymentDetailList.get(0)
									.getBranchName().replace("\n", "")
									.replace("\r", "").trim();
							writer.append(nameOfBranch != null ? nameOfBranch
									.replace(',', '-') : "");
							writer.append(',');
