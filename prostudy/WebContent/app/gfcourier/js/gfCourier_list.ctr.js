angular
		...
		.controller(
				...
				function($scope, $window, $mdToast, $timeout, $mdSidenav,
						...
						tableTestDataFactory, appEndpointSF, ajsCache) {
					...
					$scope.pendingPast8Days = function(cheked) {
						if (cheked) {
							...
							$scope.gfCouriertListBackup.forEach(function(
									courier) {
								var courierDate = new Date(
										courier.courierDispatchDate);
								if (courierDate < eightDaysBefore && !courier.courierDocketID) {
									$scope.gfCouriertList.push(courier);
								}
							});
						} else {
